...
public class PinChange extends JFrame implements ActionListener {
    ...
    PinChange(String pinnumber) {
        ...
        text.setBounds(260, 180, 200, 30);
        image.add(text);

        JLabel pintext = new JLabel("New PIN");
        pintext.setForeground(Color.WHITE);
        pintext.setFont(new Font("Raleway", Font.BOLD, 20));
        pintext.setBounds(160, 250, 150, 28);
        image.add(pintext);

        pin = new JPasswordField();
        pin.setFont(new Font("Raleway", Font.BOLD, 16));
        pin.setBounds(320, 250, 180, 28);
        image.add(pin);

        JLabel repintext = new JLabel("Re-enter PIN");
        repintext.setForeground(Color.WHITE);
        repintext.setFont(new Font("Raleway", Font.BOLD, 20));
        repintext.setBounds(160, 290, 180, 28);
        image.add(repintext);

        repin = new JPasswordField();
        repin.setFont(new Font("Raleway", Font.BOLD, 16));
        repin.setBounds(320, 290, 180, 28);
        image.add(repin);

        change = new JButton("CHANGE");
        change.setBounds(400, 360, 100, 30);
        change.addActionListener(this);
        image.add(change);

        back = new JButton("BACK");
        back.setBounds(400, 395, 100, 30);
